...
export function useVoiceAlert() {
  ...
  const playZoneAlert = useCallback(async (zoneId: string) => {
  ...
  const playCustomAlert = useCallback(
    async (province: string, district: string, severity: FloodSeverity) => {
      abortRef.current = false;
      setState({ isPlaying: false, isLoading: true, error: null, source: null });
