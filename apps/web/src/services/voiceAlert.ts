...
async function getCachedAudio(key: string): Promise<string | null> {
  try {
    ...
    return new Promise((resolve) => {
      ...
      req.onsuccess = () => {
        const record = req.result as { key: string; audioBase64: string; expiresAt: number } | undefined;
...
async function cacheAudio(key: string, audioBase64: string, ttlHours = 72): Promise<void> {
  try {
    const db = await openDB();
...
export async function fetchVoiceAlert(zoneId: string): Promise<string | null> {
  ...
  try {
    ...
    const data = await res.json() as {
      data: { audioBase64: string; contentType: string } | null;
      error: null | { message: string };
    };

    if (!data.data?.audioBase64) return null;
